...
void run(const fs::path &sequence_path) {
  ...
  for (unsigned nframes : possible_nframes) {
    ...
    while (i != trajectory.cend()) {
      ...
      //if (avgA_error > 5e-3) {
        ...
        
        {
          ...
          iterative(input, iterative_result, 1., Eigen::Isometry3d::Identity(), nullptr, false);
    ...
    std::string iterative_file = StringPrintf("%s_%d_iterative_.csv", sequence_name.c_str(), nframes);
